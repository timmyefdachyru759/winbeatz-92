...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-b3c7e96e";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-b3c7e96e">'+'<span class="tile-face-b3c7e96e tile-front-b3c7e96e"></span>'+'<span class="tile-face-b3c7e96e tile-back-b3c7e96e"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-b3c7e96e")){return;}
var back=tile.querySelector(".tile-back-b3c7e96e");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-b3c7e96e");if(isMine){tile.classList.add("boom-b3c7e96e");}
if(dim){tile.classList.add("dimmed-b3c7e96e");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-b3c7e96e")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-b3c7e96e");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-b3c7e96e")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-b3c7e96e");void panelEl.offsetWidth;panelEl.classList.add("jolt-b3c7e96e");}
endRound();}
